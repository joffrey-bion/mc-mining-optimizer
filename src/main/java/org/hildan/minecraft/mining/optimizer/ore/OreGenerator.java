...
public class OreGenerator {
     ...
     */
    public void generateInto(Sample sample, int sampleYPosition) {
    ...
    private void genStandardOre(Sample sample, BlockType type, int xOffset, int zOffset) {
        IntBinaryOperator yGenerator = (minY, maxY) -> random.nextInt(maxY - minY) + minY;
    ...
    private void genLayeredOre(Sample sample, BlockType type, int xOffset, int zOffset) {
        IntBinaryOperator yGenerator = (minY, maxY) -> 2 * random.nextInt(maxY) + (minY - maxY);
        generateOre(sample, type, xOffset, zOffset, yGenerator);
    }

    private void generateOre(Sample sample, BlockType type, int xOffset, int zOffset, IntBinaryOperator yGenerator) {
        ...
        for (int i = 0; i < type.getVeinsCountPerChunk(); ++i) {
            ...
            int y = yGenerator.applyAsInt(minY, maxY);
